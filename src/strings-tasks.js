...
function removeLastOccurrences(str, value) {
  ...
  const c = str.slice(a + value.length);
  const d = b.concat(c);
  return d;
}
...
function sumOfCodes(str) {
  ...
  const stringToWord = str.split('');
  const wordToNumber = stringToWord.map((char) => char.charCodeAt(0));
  const result = wordToNumber.reduce((current, previous) => previous + current);
...
function startsWith(str, substr) {
  ...
  //   return str;
  // }
  // const b = str.slice(a, substr.length + a);
  // if(String(b) === substr) {
  //   return true;
  // }
  // return false;
...
function containsSubstring(str, substring) {
  ...
  // const a = str.indexOf(substring);
  // if (a === -1) {
  //   return false;
  // }
  // const b = str.slice(a, substring.length + a);
...
function countVowels(str) {
  ...
  for (let index = 0; index <= str.length; index += 1) {
    if (vowelLetters.includes(str[index])) {
      result += Number(1);
    }
  }
  return result;
...
function reverseWords(str) {
  ...
  const arrReverse = [];
  arr.forEach((elem) => {
    arrReverse.push(String(elem).split('').reverse().join(''));
  });
...
function extractNameFromTemplate(value) {
  ...
  const subString2 = '!';
  const newString = string.replace(subString, '');
  const result = newString.replace(subString2, '');
  return result;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (letter) => n[a.indexOf(letter)]);
}
